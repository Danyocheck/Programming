...
void
print_max_str(char *arr[])
{
    int max_len = 0;
    int i = 0;
    while (arr[i] != NULL) { // looking for the maximum of the lengths
        char *help_arr = arr[i];
        if (strlen(help_arr) > max_len) {
            max_len = strlen(help_arr);
            //printf("%d", max_len);
        }
        ++i;
    }
    i = 0;
    while (arr[i] != NULL) { // print words with maximum lengths
        char *help_arr = arr[i];
        if (strlen(help_arr) == max_len) {
            printf("%s\n", arr[i]);
        }
