...
solution(int fsm[], unsigned fsm_size)
{
    ...
    printf("int mark, condition = 0;\n\t");
    printf("int c = 0;\n\t");
    printf("long long h = 0;\n\t");
    printf("scanf(\"%%d\", &mark);\n\t");
    printf("while (mark != 0) {\n\t\t");
    printf("h = (((unsigned long long) mark) << 32) | (int) condition;\n\t\t");
    printf("switch (h)\n\t\t{\n\t\t");
    for (int i = 1; i < fsm_size * 3 - 1; i += 3) {
        printf("case (((((unsigned long long) ");
        printf("%d ", fsm[i]);
        printf(") << 32) | (int) ");
        printf("%d ", fsm[i - 1]);
        printf(")) : condition = ");
    ...
    printf("default: c = 1; break;}\n\t\t");
    printf("if (c == 1)\n\t\t\tbreak;\n\t\t");
    printf("scanf(\"%%d\", &mark);\n\t}\n\t");
    printf("if ((c == 0) && (condition == 1))\n\t\tprintf(\"YES\");\n\telse\n\t\tprintf(\"NO\");\n}");
}
...
main(void)
{
    static const char source[] = "template.c";
    static const char binary[] = "./template";
